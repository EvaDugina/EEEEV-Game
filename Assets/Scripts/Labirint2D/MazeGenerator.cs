...
public class MazeGenerator
{
    ...
    public MazeGenerator(int areaId, int width, int height, MazeStructure structure)
    {
        AreaId = areaId;
...
|   Methods
    ...
    */
    ...
    public Maze GenerateMainMaze(StaticPositionParameter start, DynamicPositionParameter finish)
    {
        ...
        string name = AreaId.ToString() + Maze.GetMazeSideAsText(mazeSide);
        ...
        Maze maze = new Maze(name, width, height, mazeSide);
        maze.Cells = Cells;
    ...
    {
        string name = AreaId.ToString() + "_" + Maze.GetMazeSideAsText(side);
        ...
        Maze boundaryMaze = new(name, cells.Length, cells[0].Length, side)
        {
            Cells = cells
        };
    ...
    private void GenerateGeometry()
    {
        if (Structure.Form == MazeForm.Triangle)
            Cells = MazeUtilities.DefineTriangleMaze(Width, Height);
        else
            Cells = MazeUtilities.DefineDefaultMaze(Width, Height);
    }
    ...
    private void RemoveWall(MazeCell a, MazeCell b)
    {
        if (a.X == b.X)
        {
            if (a.Y > b.Y) a.WallsStatus.BottomWall = false;
            else b.WallsStatus.BottomWall = false;
        }
        else if (a.Y == b.Y)
        {
            if (a.X > b.X) a.WallsStatus.LeftWall = false;
            else b.WallsStatus.LeftWall = false;
        }
    }
    ...
    public void CreateBoundaryPasseges()
    {
        ...
        for (int y = 0; y < Height - 1; y++)
        {
            ...
            if (Cells[0][y].WallsStatus.LeftWall == Cells[xMax][y].WallsStatus.LeftWall && y % 2 == 1)
            {
                Cells[0][y].WallsStatus.LeftWall = false;
                Cells[xMax + 1][y].WallsStatus.LeftWall = false;
            }
        }
        ...
        for (int x = 0; x < Width - 1; x++)
        {
            ...
            //    startIndex = -1;
            //}
            if (Cells[x][0].WallsStatus.LeftWall == Cells[x][yMax].WallsStatus.LeftWall && x % 2 == 1)
            {
                Cells[x][0].WallsStatus.BottomWall = false;
                Cells[x][yMax + 1].WallsStatus.BottomWall = false;
            }
        }
    }
